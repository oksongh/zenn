...
追記（2026年2月1日）:
[不変な初期化と式指向の相性がよい](#不変な初期化と式指向の相性がよい)の条件式を等価に変換する箇所が等価でなかったため修正しました。
...
(個人的には`let x = 式;`で初期化が完了することを「一撃で初期化」などと呼んでいる ~~のだが、逆に、式指向による「一撃で初期化」がなかったら、変数宣言のたびに`mut`をつけて回らなくてはならず、変数宣言のデフォルトが不変にならなかったのではないか？と想像している。~~ )

追記：
コメントで指摘いただきましたが、誤りがありました。
* 変数を不変に初期化するにはRustでは`let x = 式;`の形にする必要がある。
* 「一撃で初期化」がなかったら、変数宣言のたびに`mut`をつけて回らなくてはならず、変数宣言のデフォルトが不変にならなかったのではないか

は誤りで、`let x; x = 式;`(`mut`不要)とも書けました。
...
Javaも`final var x = 式;`、`final var x; x = 式;`で書けます。
```java
public class Main {
    public static void main(String[] args) {
...
(個人的には良いと思うが)`ブロックで囲む`はコーディング規約で認められるのか微妙で、シャドーイングのような機能はないので変数に`tmp`などを付ける必要がある。
`条件の組み替え`はロジックの検証コストがかかる。
...
#### 条件の組み替え
`str`への代入を一度だけにすると`final`を追加できる。
追記：以前書いたものは等価なプログラムではなかったため修正。
...
### Rustでの実例
Rustでそのまま書くと以下となる。
不変にする方法はいくつかあるが、`let x = 式;`の形式で初期化できるのは`ブロックで囲む`、`条件の組み替え1`、`条件の組み替え2`、`シャドーイング`、`match式`(後述)。この例で手頃なのは`ブロックで囲む`と`シャドーイング`、網羅性が高いのは`match式`。この例だと`条件の組み替え2`ではネストが深くなってしまったが、if式での初期化も`ブロックで囲む`と同様に使える場面もある。
Javaと比べたRustの文法のメリットは、`ブロックで囲む`が使えることだと思う。
...
追記：
コメントで指摘いただいて初めて知ったが、漏れなく初期化できればデフォルト値がなくても合法。
...
::: 間違えていた以前のコード
（以前ここでコンパイルエラーが出ると書いていましたが、再度確認したらコンパイルでき、さらに条件の組み替えを間違えていて、`mut`も不要だったので訂正します。すみませんでした。）
...
fn foo() {
    ...
    let str: String = match (flag, flag2, flag3) {
        (true, false, false) => "foo".to_string(), // flagがtrue,flag2がfalse,flag3がfalseの場合は"foo"
        (_, true, _) => "bar".to_string(),         // flag2がtrueの場合は"bar"
        (_, false, true) => "qux".to_string(),     // flag2がfalse,flag3がtrueの場合は"qux"
